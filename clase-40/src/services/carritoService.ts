...
const obtener = async (id: string): Promise<CarritoDTO> => {
  try {
    return await carritoDAO.get(id);
  } catch (e) {
...
const getByOwnerEmail = async (email: string): Promise<CarritoDTO> => {
  try {
    return await carritoDAO.getBy("owner", email);
  } catch (e) {
...
const deleteByEmail = async (email: string): Promise<void> => {
  ...
  await carritoDAO.deleteBy("owner", email);
};
